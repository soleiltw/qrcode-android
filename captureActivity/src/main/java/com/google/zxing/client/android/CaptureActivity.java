...
public class CaptureActivity extends Activity implements SurfaceHolder.Callback
{
    ...
    public void onCreate(Bundle icicle)
    {
        ...
        // PR:LivotovLabs/zxscanlib 註解掉這段就不會強迫在掃QR code時用landscape了(但同時也要設定camera的orientation，不然在CaptureActivity為portrait的情況下camera會是landscape)
...
//        }
    ...
    public void handleDecode(Result rawResult, Bitmap barcode)
    {
        ...
        if (accept)
        {
            handleDecodeExternally(rawResult, barcode); // PROC 1
        } else {
        	Toast.makeText(this, "please scan another QR code", Toast.LENGTH_SHORT).show(); // JTODO string xml
